package com.taikang.tkdoctor.activity.widget.ruler;

import com.taikang.tkdoctor.R;
import com.taikang.tkdoctor.activity.widget.ruler.RulerScrollView.ScrollType;
import com.taikang.tkdoctor.activity.widget.ruler.RulerScrollView.ScrollViewListener;
 ...
 * 
 */
public class RulerBloodSugar extends FrameLayout {
	...
	public RulerBloodSugar(Context context, AttributeSet attrs, int defStyleAttr) {
		super(context, attrs, defStyleAttr);

		TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.Ruler,
				defStyleAttr, 0);
		minUnitSize = a.getDimension(R.styleable.Ruler_min_unit_size, 20.0f);//最小单位大小
		maxUnitCount = a.getInteger(R.styleable.Ruler_max_unit_count, 24);//尺寸
	...
	public RulerBloodSugar(Context context, AttributeSet attrs) {
		super(context, attrs, R.attr.ruler_style);

		TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.Ruler,
				R.attr.ruler_style, 0);
		minUnitSize = a.getDimension(R.styleable.Ruler_min_unit_size, 20.0f);//竖线之间的间隔大小
	...
	public RulerBloodSugar(Context context) {
		super(context, null);
		init();
	...
	private void initParentContainer() {
		...
		FrameLayout.LayoutParams scrollerParams = new FrameLayout.LayoutParams(-1, height);
//		scrollerParams.gravity = Gravity.CENTER_VERTICAL;
//		scrollerParams.gravity = Gravity.BOTTOM;
		//顶部位置
...
//		params2.addRule(RelativeLayout.ABOVE, R.id.unit_container_id);//设置刻度尺的位置 在刻度尺数值之上
		...
		unitContainer.setId(R.id.text_bottom_id);
//		unitContainer.setPadding(dp2px((int) unitPadding), 0, dp2px((int) unitPadding), 0);
		unitContainer.setPadding(dp2px((int) unitPadding), 0, dp2px((int) unitPadding), 0);
		rulerContainer.addView(unitContainer);
		RelativeLayout.LayoutParams paramsTop = new RelativeLayout.LayoutParams(-1, -2);
//		paramsTop.topMargin = dp2px((int) (bmpMaxHeight / 2 + resultTextSize));
		paramsTop.topMargin = dp2px((int) (bmpMaxHeight / 2 +resultTextSize));
//		paramsTop.rightMargin=dp2px(0);
		paramsTop.leftMargin=dp2px(0);//控制标尺字符的位置
		textContainer = new LinearLayout(getContext());
		textContainer.setLayoutParams(paramsTop);//设置标尺整个刻度尺上数值
		textContainer.setOrientation(LinearLayout.HORIZONTAL);
//		textContainer.setId(R.id.unit_container_id);//设置文字的ID
...
//		rulerContainer.addView(unitContainer);
		...
		//resultContainer是放置左右刻尺Demo 右侧数值
		//貌似是当前刻度值
...
//				paramsTop.topMargin);
		...
		resultTagView.setVisibility(View.GONE);
...
//				这部分padding计算的总规则是，宽度的一半减去unitContainer左边的padding，再减去unitContainer第一个刻度的一半宽度。
	...
	public void setDefaultExtra(float extra){
	...
	private void initUnit() {
		...
		for (int i = 0; i <= maxUnitCount * 2; i++) {
			...
			if(i<maxUnitCount*2-1){
				...
				if(i<18){
					textUnitView.setPadding(0, 0, 0, 0);
				}
				if(i==18){
					textUnitView.setPadding(0, 0, 0, 0);
				}
				if(i==19){
					LinearLayout.LayoutParams	maxParams2 = new LinearLayout.LayoutParams(dp2px((int) minUnitSize) * 4+1, -2);
					textUnitView.setLayoutParams(maxParams2);
				}
			}else if(i<maxUnitCount*2){
				LinearLayout.LayoutParams param = new LinearLayout.LayoutParams(dp2px((int) minUnitSize)*3+12, -2);
				textUnitView.setLayoutParams(param);
			}else{
				LinearLayout.LayoutParams param = new LinearLayout.LayoutParams(dp2px((int) minUnitSize)*3+14, -2);
				param.leftMargin=20;
//				textUnitView.setPadding(10, 0, 0, 0);
				textUnitView.setLayoutParams(param);
			}
			textUnitView.setGravity(Gravity.BOTTOM | Gravity.LEFT);
			if (i % 2 == 0) {
				textUnitView.setText(String.format("%.1f", (float)(i / 2)));
			}
	...
	private void initDrawable() {
		int maxHeight = dp2px((int) bmpMaxHeight);//最长竖线的高度
		int midHeight = maxHeight * 3 / 4;//中等高度竖线的高度
		int minHeight = maxHeight * 2 / 3;//最短竖线的高度
...
//		canvas4.drawLine(0, 0, 0, markBgBmp.getHeight(), paint);
		canvas4.drawLine(0, 0, 0, maxHeight, paint);
	}
	...
	private void showResult(int max, int min, float val) {
		...
		if (mode == MODE_RULER) {
			resultView.setText(String.format("%.01f",
